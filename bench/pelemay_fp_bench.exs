defmodule PelemayFpBench do
  ...
  bench "PelemayFp with chunk size 100000 (6 cores even)" do
    [{"list", list}] = :ets.lookup(:my_table, "list")
  	PelemayFp.map(list, &LogisticMap.logistic_map_10(&1), 100000)
  end

  bench "PelemayFp with chunk size 120000 (5 cores even)" do
    [{"list", list}] = :ets.lookup(:my_table, "list")
    PelemayFp.map(list, &LogisticMap.logistic_map_10(&1), 120000)
  end

  bench "PelemayFp with chunk size 150000 (4 cores even)" do
    [{"list", list}] = :ets.lookup(:my_table, "list")
    PelemayFp.map(list, &LogisticMap.logistic_map_10(&1), 150000)
  end

  bench "PelemayFp with chunk size 200000 (3 cores even)" do
    [{"list", list}] = :ets.lookup(:my_table, "list")
    PelemayFp.map(list, &LogisticMap.logistic_map_10(&1), 200000)
  end

  bench "PelemayFp with chunk size 300000 (2 cores even)" do
    [{"list", list}] = :ets.lookup(:my_table, "list")
    PelemayFp.map(list, &LogisticMap.logistic_map_10(&1), 300000)
  end

  bench "PelemayFp with chunk size 600000 (1 cores even)" do
    [{"list", list}] = :ets.lookup(:my_table, "list")
    PelemayFp.map(list, &LogisticMap.logistic_map_10(&1), 600000)
  end
  ...
  bench "PelemayFp and Pelemay with chunk size 50000 (12cores even)" do
    [{"list", list}] = :ets.lookup(:my_table, "list")
    PelemayFp.map_chunk(list, &LogisticMap.logistic_map_10(&1), &LogisticMap.logistic_map_10_pelemay(&1), 50000)
  end

  bench "PelemayFp and Pelemay with chunk size 100000 (6cores even)" do
    [{"list", list}] = :ets.lookup(:my_table, "list")
    PelemayFp.map_chunk(list, &LogisticMap.logistic_map_10(&1), &LogisticMap.logistic_map_10_pelemay(&1), 100000)
  end

  bench "PelemayFp and Pelemay with chunk size 120000 (5cores even)" do
    [{"list", list}] = :ets.lookup(:my_table, "list")
    PelemayFp.map_chunk(list, &LogisticMap.logistic_map_10(&1), &LogisticMap.logistic_map_10_pelemay(&1), 120000)
  end

  bench "PelemayFp and Pelemay with chunk size 150000 (4cores even)" do
    [{"list", list}] = :ets.lookup(:my_table, "list")
    PelemayFp.map_chunk(list, &LogisticMap.logistic_map_10(&1), &LogisticMap.logistic_map_10_pelemay(&1), 150000)
  end

  bench "PelemayFp and Pelemay with chunk size 200000 (3cores even)" do
    [{"list", list}] = :ets.lookup(:my_table, "list")
    PelemayFp.map_chunk(list, &LogisticMap.logistic_map_10(&1), &LogisticMap.logistic_map_10_pelemay(&1), 200000)
  end

  bench "PelemayFp and Pelemay with chunk size 300000 (2cores even)" do
    [{"list", list}] = :ets.lookup(:my_table, "list")
    PelemayFp.map_chunk(list, &LogisticMap.logistic_map_10(&1), &LogisticMap.logistic_map_10_pelemay(&1), 300000)
  end

  bench "PelemayFp and Pelemay with chunk size 600000 (1cores even)" do
    [{"list", list}] = :ets.lookup(:my_table, "list")
    PelemayFp.map_chunk(list, &LogisticMap.logistic_map_10(&1), &LogisticMap.logistic_map_10_pelemay(&1), 600000)
  end
